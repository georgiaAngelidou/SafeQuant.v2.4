...
#' @export
safeQuantAnalysis <- function(eset=eset, method=c("global","naRep","pairwise", "median"), intensityAdjustmentObj=NA, fcThrs=1, log2v=TRUE){
	...
	if(class(intensityAdjustmentObj) == "list"){
	  ...
	  if ('mean' %in% method){
	  # Geo: By default while calculate the log ration by the median. To change it add method=mean
	  out$ratio <- getRatios(eset,log2=T, method='mean')
	  }else{
	    out$ratio <- getRatios(eset,log2=T)
	  }
	}
