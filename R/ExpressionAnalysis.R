...
createExpressionDataset <- function(expressionMatrix=expressionMatrix,expDesign=expDesign,featureAnnotations=featureAnnotations){
  ...
  if(length(unique(as.character(expDesign$c_g[expDesign$isControl])) ) > 1){
...
#' @examples print("No examples")
getAllCV <- function(eset, l2=TRUE){
...
#' @examples print("No examples")
# Geo:  Here Normalize the values base on median
...
sqImpute = function(eset,method="gmin", rowmax=0.3){
  ...
  if(method == "gmin"){
    ...
    exprs(esetImp)[  is.na(exprs(esetImp)) ] <- 0
    exprs(esetImp) = exprs(esetImp) +  rowImp
  }else if (method == "ndist"){
    ese_log = log2(exprs(esetImp))
    df_mean = colMeans(ese_log, na.rm = T)
    df_sd   = apply(ese_log,2, sd, na.rm=T)
    df_mean2 = df_mean - (1.8*df_sd)
    tmp <- sapply(1:ncol(ese_log), function(x) rnorm(nrow(esetImp), mean = df_mean2[x], sd = 0.3*df_sd[x]))
    tmp_2 <- 2^tmp
    exprs(esetImp)[is.na(exprs(esetImp))] <- tmp_2[is.na(exprs(esetImp))]
  }else{
