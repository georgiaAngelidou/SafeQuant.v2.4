...
proteinTable_reformat <- function(filePath, df2, df3){
  ...
  fileType <- df$File_origin[1]
  if (fileType %in% c("Spectronant_peptide")){
    df_sm <- df[,c("Protein.Ids", "Genes", "Razor_genes", "Global.PG.Q.Value")]
    df_sm$razor <- ""
    df3_sm <- df3[grepl('NrOfPrecursors', names(df3))]
    proteinNames <- df3$proteinName
    proteinIds <- df3$proteinName
    df3_sm <- cbind(proteinNames, proteinIds, df3_sm)
    df3_sm$razor <- ""
    names_ch <- names(df3_sm)[grepl('NrOfPrecursors', names(df3_sm))]
    ...
    neworder <- c("proteinName", "Other_proteins", "geneName", "Other_geneNames", "proteinDescription", "Global.PG.Q.Value", names_ch)
